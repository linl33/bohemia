  ...
  chunk_list[[i]] <-
    ...
    # We use the previously enumerated data then
      this_df <- enumerations_data %>% dplyr::filter(hamlet_code == lc) %>% dplyr::select(
        agregado, hamlet, hamlet_code, localizacao_agregado, todays_date)
      # Add team numbers
      nr = nrow(this_df)
      if(nr > 0){
