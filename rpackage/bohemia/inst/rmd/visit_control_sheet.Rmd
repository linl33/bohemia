...
if(country == 'Mozambique'){
  ...
  chunk_list[[i]] <-
    ...
    # We use the previously enumerated data then
      this_df <- enumerations_data %>% dplyr::filter(hamlet_code == lc) %>% dplyr::select(
        agregado, hamlet, hamlet_code, localizacao_agregado, todays_date, chefe_name) %>%
  ...
  chunk_list[[i]] <-
    paste0("Team ", i, "\n\n\nFieldworker name/code:\n\n\n```{r}\n",
           "kable(out %>% filter(team == ", i, ") %>% tibble %>% dplyr::select(-team) %>% dplyr::mutate(`Status` = ' ', `Comments` = ' '), 'latex', longtable = T, booktabs=FALSE, caption = 'Team ", i,"')  %>%   kable_styling(font_size = 7) %>% kable_styling(latex_options = c( 'repeat_header')) %>% column_spec(1:6, width = '1.5cm') %>% column_spec(7, width = '3cm')   \n```\n\\newpage\n")
}
}
