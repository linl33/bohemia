...
if(country == 'Mozambique'){
  ...
  chunk_list[[i]] <-
    paste0("## Team ", i, "\n\n\nNome/Código do Inquiridor:\n\n\n```{r}\n",
           "kable(out %>% filter(team == ", i, ") %>% tibble %>% dplyr::select(-team) %>% dplyr::mutate(`Nome de chefe de agregado` = '", "', `Localização do Numero de Agregado` = '",   ' ', "', `Data de enumeração` = ' '), 'latex', longtable = T, booktabs=TRUE, caption = 'Team ", i,"')  %>%   kable_styling(font_size = 9) %>% kable_styling(latex_options = c('striped', 'repeat_header')) %>% column_spec(1:4,width = '2.5cm')   \n```\n\\newpage\n")
}
  } else {
    # NON ENUMERATION
      this_df <- gps %>% dplyr::filter(code == lc) %>% left_join(locations %>% dplyr::select(code,Village, Hamlet))
