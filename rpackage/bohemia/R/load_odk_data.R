...
load_odk_data <- function(the_country = 'Mozambique',
  ...
  if(efficient){
    if(!is.null(the_country)){
      ...
      refusals <- dbGetQuery(con, "SELECT instance_id, hh_geo_location,country, reason_no_participate,hamlet,district,region, hamlet_code, hh_id, hh_region, village, ward, todays_date, wid FROMclean_refusals")
    }
  } else {
    if(!is.null(the_country)){
