...
# Get polygon
shp <- shp_poly <- bohemia::ruf3
shp <- fortify(shp, region = 'NAME_3')
...
# Get roads, water, etc.
roads <- roads_shp <- bohemia::rufiji_roads
water <- water_shp <- bohemia::rufiji_water
waterways <- waterways_shp <- bohemia::rufiji_waterways
roads <- fortify(roads)
water <- fortify(water)
...
for(i in 1:nrow(shp_poly)){
  ...
  this_id <- this_shp_poly$NAME_3
  this_shp <- shp %>% filter(id == this_id)
