...
ages <- 
  ...
  mutate(is_child  = years_old >= 5 & years_old <= 15) %>%
  group_by(country) %>%
...
We consider that a "child" is some between ages 5 and 15. We consider that the % of people which are children will be `r paste0(ages$percent_children, ' in ', ages$country, collapse = ', ')` (as per the data observed so far in the minicensus).
...
If we keep only the minimal number required (`r n_required` clusters) and do not cense those in buffer areas, this is estimated to reduce the number of participating households from `r sum(out_maximal$n_households)` to `r sum(out_minimal$n_households)`.
