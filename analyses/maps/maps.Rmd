...
```{r}
# Health facilities map for MOZ
hf_moz <- bohemia::mopeia_health_facilities
hf_moz

ggplot() +
  geom_polygon(data = bohemia::mop2_fortified,
               aes(x = long,
                   y = lat, 
                   group = group),
               fill = 'lightblue') +
  geom_point(data = bohemia::mopeia_health_facilities,
             aes(x = lng,
                 y = lat),
             size = 3) +
  geom_point(data = tibble(lng = 35.711553,
                         lat = -17.979446,
                         name = 'HQ'),
             color = 'red',
             pch = 11,
             aes(x = lng,
                 y = lat)) +
  ggrepel::geom_text_repel(data = bohemia::mopeia_health_facilities,
                           aes(x = lng,
                               y = lat,
                               label = name),
                           size = 2) +
  theme_solid()
ggsave('~/Desktop/mopeia_health_facilities.png')


library(leaflet)
leaflet()  %>%
  addProviderTiles(providers$Esri.WorldImagery) %>%
  addPolylines(data = bohemia::mop2) %>%
  addMarkers(data = bohemia::mopeia_health_facilities,
             popup = bohemia::mopeia_health_facilities$name)

```


```{r}
# Clinical trial map for TZA
...
```{r}
ggplot() +
  geom_polygon(data = bohemia::ruf2_fortified,
               aes(x = long,
                   y = lat, 
                   group = group),
               fill = 'beige') +
  geom_point(data = gps_tza@data %>%
               mutate(ct = ifelse(clinical_trial == 1, 'Clinical trial', 'No clinical trial')),
             aes(x = lng,
                 y = lat,
                 color = ct),
             size = 1, 
             alpha = 0.6) +
  # ggrepel::geom_text_repel(data = gps_tza@data,
  #                          aes(x = lng,
  #                              y = lat,
  #                              label = code),
  #                          size = 1) +
...
ggsave('~/Desktop/clinical_trials.png')
```

# Same as above but add locations of health facilities sent by Mwaka

```{r}
library(readxl)
mwaka <- read_excel('data/Copy of 20200812 IHI Health Facilities in Kibiti and Rufiji - Amended.xlsx', skip = 1)
mwaka2 <- read_excel('data/Copy of 20200812 IHI Health Facilities in Kibiti and Rufiji - Amended.xlsx', skip = 1, sheet = 2)
mwaka2$bohemia <- TRUE
mwaka$bohemia <- NA
mwaka <- bind_rows(
  mwaka2,
  ...
  mutate(id = paste0(`Facility Number`, `Facility Name`, `Date Opened`)) %>%
  dplyr::distinct(id, .keep_all = TRUE) %>%
  mutate(bohemia = ifelse(is.na(bohemia), FALSE, bohemia)) %>%
  mutate(bohemia = ifelse(bohemia, 'Bohemia', 'Not Bohemia'))
...
ggplot() +
  ...
  theme_map() +
  scale_color_manual(name = '',
                     values = c('red', 'green', 'darkred', 'darkgreen')) +
  theme(legend.position = 'bottom',
        legend.text = element_text(color = 'black'))
ggsave('~/Desktop/clinical_trials_with_hf.png')
```
...
ggplot() +
  ...
  theme(legend.position = 'bottom',
        legend.text = element_text(color = 'black')) +
  geom_point(data = bohemia::rufiji_health_facilities,
             aes(x = lng,
                 y = lat),
             size = 6, 
             color = 'blue',
...
```{r}
# See which hamlet is closest to each rufiji health facility
rufiji_health_facilities_sp <- bohemia::rufiji_health_facilities %>%
  mutate(x = lng,
         y = lat)
coordinates(rufiji_health_facilities_sp) <- ~x+y
proj4string(rufiji_health_facilities_sp) <- proj4string(gps_tza)
distances <- distm(
  x = rufiji_health_facilities_sp,
  y = gps_tza
)

closest <- apply(distances, 1, which.min)
nearest_hamlet <- gps_tza@data$code[closest]
clinical_trial <- gps_tza@data$clinical_trial[closest]

rufiji_health_facilities_sp@data$nearest_hamlet <- nearest_hamlet
rufiji_health_facilities_sp@data$clinical_trial <- clinical_trial
```
...
ggplot() +
  ...
  theme(legend.position = 'bottom',
        legend.text = element_text(color = 'black')) +
  geom_point(data = rufiji_health_facilities_sp@data %>%
               mutate(ct = ifelse(clinical_trial == 1, 'Clinical trial', 'No clinical trial')),
             aes(x = lng,
                 y = lat,
                 color = ct),
...
```{r}
rf <- rufiji_health_facilities_sp@data %>%
               mutate(ct = ifelse(clinical_trial == 1, 'Clinical trial', 'No clinical trial')) %>% dplyr::select(-clinical_trial)

write_csv(rf, '~/Desktop/estimated_clinical_trial_status_of_rufiji_health_facilities.csv')
```
...
leaflet()  %>%
  addProviderTiles(providers$Esri.WorldImagery) %>%
  addPolylines(data = bohemia::ruf2) %>%
  addCircleMarkers(data = gps_tza@data %>% filter(clinical_trial == 0),
                   color = 'black', popup = gps_tza@data$code[gps_tza@data$clinical_trial == 0]) %>%
  addCircleMarkers(data = gps_tza@data %>% filter(clinical_trial == 1),
                   color = 'red', popup = gps_tza@data$code[gps_tza@data$clinical_trial == 1])
