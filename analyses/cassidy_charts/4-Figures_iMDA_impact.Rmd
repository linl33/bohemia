...
m2 = tm_shape(World, bbox = bhu_bb) +  
  ...
  tm_shape(study_area, bbox = bhu_bb) +
    ...
    tm_text("iso_a3", size = 0.6) +
    tm_layout(frame = T, legend.position = c("left", "bottom"), bg.color="lightcyan1",
              legend.title.size=1.5, legend.text.size = 0.5) 
...
## Figures 3: Cumulative averted financial costs due to BOHEMIA ivermectin MDA for all 10 countries.
Given a Griffin scenario, the treatment costs averted due to BOHEMIA iMDA. These figures show the marginal effect of BOHEMIA iMDA. Note that, the calculation does not account for money saved from deaths averted.\
```{r, results = "asis", echo=F, fig.width=10, fig.height=6, fig.fullwidth=TRUE}

png("./output/Figures_online/1.4_averted_cost_all.png", 700, 1000)
par(mfrow=c(3,1))
for (uptake.id in 1:3){
  bardata<-NULL
  for (grif.rt.id in 1:5){
  ...
  bp<-barplot(bar_mat/1000000,horiz = TRUE,beside = T,xlim=c(0, 2250), cex.axis = 1.5, cex.lab = 1.5, cex.main=1.5,
              legend.text = c('20% Efficacy, 1% severe cases','20% Efficacy, 3% severe cases',
                              '40% Efficacy, 1% severe cases','40% Efficacy, 3% severe cases'),
              names.arg = grif.name[c(4,5,1,2,3)], yaxt = 'n', xlab= 'Dollars (million)',
              args.legend = list(x = "topright",bty = 'n', cex=1.7), 
              col = c("#E69F00", "black", "lightgrey","#4100E6"),
              main = paste0(uptake.name[uptake.id],' Uptake'))
      axis(2, at=apply(bp,2,mean), labels=grif.name[c(4,5,1,2,3)], tick=FALSE, las=2, line=-0.8, cex.axis = 1.4, cex.lab = 2)
      text(y = bp, x = bar_mat/1000000, label = signif(bar_mat/1000000,3), pos = 4, cex = 1.5, col = "red")
}
...
```{r, results = "asis", echo=F, fig.width=10, fig.height=6, fig.fullwidth=TRUE}
png(paste0("./output/Figures_online/1.3_averted_deaths_all.png"), 600, 1000)
par(mfrow=c(3,1))
for (uptake.id in 1:3){
  avertdata<-NULL
  for (grif.rt.id in 1:5){
  ...
  par(mar=c(4.1,0,1.1,2))
  bp<-barplot(avert_matdeath/1000,horiz = TRUE,beside = T, xlim=c(0,590),ylim=c(0,18), cex.axis = 1.5, cex.lab = 1.5, cex.main=1.5, axisnames=FALSE,
              legend.text = c('20% Efficacy','40% Efficacy'),
              yaxt = 'n', xlab= 'Number of averted deaths in thousand',
              args.legend = list(x = "topright",bty = 'n',cex = 1.7))
      axis(2, at=apply(bp,2,mean), labels=grif.name[c(4,5,1,2,3)], tick=FALSE, las=2, line=-0.8, cex.axis = 1.4, cex.lab = 2)
      text(y = bp, x = avert_matdeath/1000, label = signif(avert_matdeath/1000,3), pos = 4, cex = 1.5, col = "red")
}
...
Given a Griffin scenario, the number of deaths averted due to the BOHEMIA iMDA.\
```{r, results = "asis", echo=F, fig.width=10, fig.height=6, fig.fullwidth=TRUE}
png(paste0("./output/Figures_online/1.5_averted_deaths_all.png"), 600, 1000)
par(mfrow=c(3,1))
for (uptake.id in 1:2){
  avertdata<-NULL
  avertdata.case <- NULL
  for (grif.rt.id in 1:5){
    avertdata.temp<-NULL
  ...
  par(mar=c(4.1,6.8,1.1,0.5))
  bp1 = barplot(-avert_mat/1000000,horiz = TRUE,beside = T, xlim=c(-350,0), ylim=c(0,18), cex.axis = 1.5, cex.lab = 1.5, cex.main=1.5, xlab= 'Number of averted cases in million')
             
      title(paste0(uptake.name[uptake.id],' Uptake'), cex.main=1.5,adj=0, line=-2)
      axis(2, at=apply(bp1,2,mean), labels=grif.name[c(4,5,1,2,3)], tick=FALSE, las=2, line=-0.8, cex.axis = 1.4, cex.lab = 2)
      text(y = bp1, x = -avert_mat/1000000-25, label = signif(avert_mat/1000000,3), pos = 4, cex = 1.5, col = "red")
...
## Figures1.1: Change in malaria incidence for each country
```{r, results = "asis", echo=F, fig.width=10, fig.height=6, fig.fullwidth=TRUE}
figure_folder = './output/Figures_online/'
for (country.data.id in 1:9){
 
  png(paste0(figure_folder, "1.1_malaria_cases_",country.name[country.data.id],".png"), 600, 1000)
  par(mfrow=c(3,1))
  for (uptake.id in 1:3){
    casedata<-NULL
    for (grif.rt.id in 1:5){
      casedata.temp<-NULL
      for (eff.id in 1:2){
       case.temp<-data.frame(temp[[uptake.id]][[eff.id]][[grif.rt.id]][[1]][[country.data.id]][[1]])
        if (eff.id == 1){
         casedata.temp<-rbind(case.temp[,2]/case.temp[,1],case.temp[,4]/case.temp[,1])
        }else{
         casedata.temp<-rbind(casedata.temp,case.temp[,4]/case.temp[,1])
  ...
  max_y<-max(casedata[[4]])
  plot(1:dim(casedata[[1]])[2]+year.init-1, casedata[[1]][1,], type='l', col = 1, lty = 1, lwd = 2,
       ylim=c(0,max_y),ylab='Incidence per 1000 people at risk',xlab='Year', cex.main=1.5,
       cex.lab=1.5,cex.axis=1.5,main=paste0(uptake.name[uptake.id],' uptake'))
  lines(1:dim(casedata[[1]])[2]+year.init-1, casedata[[1]][2,], type='l', col = 1, lty = 2, lwd = 2)
  lines(1:dim(casedata[[1]])[2]+year.init-1, casedata[[1]][3,], type='l', col = 1, lty = 3, lwd = 2)
  for (i in 2:5){
    for (j in 1:3){
      lines(1:dim(casedata[[1]])[2]+year.init-1, casedata[[i]][j,], type='l', col = i*2, lty = j, lwd = 2)
    }
  }
  legend('bottomleft',c(grif.name[c(4,5,1,2,3)]),
         lty=1,lwd=2,bty='n',col=c(8,10,1,4,6),text.col=c(8,10,1,4,6),cex=1.5)
  legend('bottom',c('Baseline scenario','20% efficacy','40% efficacy'),
         lty=c(1:3),lwd=2,bty='n',cex=1.5)
...
## Figures1.2: Cumulative averted incidence cases due to BOHEMIA ivermectin MDA for each country
Given a Griffin scenario, the number of cases averted due to the BOHEMIA iMDA.\
```{r, results = "asis", echo=F, fig.width=10, fig.height=6, fig.fullwidth=TRUE}
for (country.data.id in 1:9){
  
  png(paste0(figure_folder, "1.2_averted_cases_",country.name[country.data.id],".png"), 600, 1000)
  par(mfrow=c(3,1))
  for (uptake.id in 1:3){
    avertdata<-NULL
    for (grif.rt.id in 1:5){
      avertdata.temp<-NULL
      for (eff.id in 1:2){
        avert.temp<-temp[[uptake.id]][[eff.id]][[grif.rt.id]][[1]][[country.data.id]][[5]]
        avertdata.temp<-c(avertdata.temp,avert.temp[length(avert.temp)])
      }
     avertdata[[grif.rt.id]]<-avertdata.temp
    }
    avert_mat<-matrix(unlist(avertdata),nrow=2,byrow=F)[,c(4,5,1,2,3)]
  par(mar=c(5.1,6.8,4.1,2.1))
  if(country.data.id ==1){
    xlim=c(0,130)
  }else if(country.data.id==2){
    xlim=c(0,70)
  }else{
    xlim = c(0,30)
  }
    bp<-barplot(avert_mat/1000000,horiz = TRUE,beside = T, xlim=xlim, ylim=c(0,18), cex.axis = 1.5, cex.lab = 1.5,  cex.main=1.5,legend.text = c('20% Efficacy','40% Efficacy'),
              names.arg = grif.name[c(4,5,1,2,3)], yaxt = 'n', xlab= 'Number of averted cases in million',
              args.legend = list(x = "topright",bty = 'n',cex=1.7), 
              main = paste0(uptake.name[uptake.id],' Uptake'))
      axis(2, at=apply(bp,2,mean), labels=grif.name[c(4,5,1,2,3)], tick=FALSE, las=2, line=-0.8, cex.axis = 1.4, cex.lab = 2)
      text(y = bp, x = avert_mat/1000000, label = signif(avert_mat/1000000,2), pos = 4, cex = 1.5, col = "red")  
}
dev.off()
}
```

## Figures1.3: Cumulative averted incidence deaths due to BOHEMIA ivermectin MDA for each country
Given a Griffin scenario, the number of deaths averted due to the BOHEMIA iMDA.\
```{r, results = "asis", echo=F, fig.width=10, fig.height=6, fig.fullwidth=TRUE}
for (country.data.id in 1:9){
  
  png(paste0(figure_folder, "1.3_averted_deaths_",country.name[country.data.id],".png"), 600, 1000)
  par(mfrow=c(3,1))
  for (uptake.id in 1:3){
    avertdata<-NULL
    for (grif.rt.id in 1:5){
      avertdata.temp<-NULL
        for (sev.case.id in 1:2){
        avert.temp<-temp[[uptake.id]][[eff.id]][[grif.rt.id]][[1]][[country.data.id]][[6]]
        avertdata.temp<-c(avertdata.temp,avert.temp[length(avert.temp)])
      }
     avertdata[[grif.rt.id]]<-avertdata.temp
    }
 avert_matdeath<-matrix(unlist(avertdata),nrow=2,byrow=F)[,c(4,5,1,2,3)]
  par(mar=c(5.1,6.8,4.1,2))
  if(country.data.id ==1){
    xlim=c(0,200)
  }else if(country.data.id==2){
    xlim=c(0,105)
  ...
  bp<-barplot(avert_matdeath/1000,horiz = TRUE,beside = T, xlim=xlim,ylim=c(0,18), cex.axis = 1.5, cex.lab = 1.5, cex.main=1.5,
              legend.text = c('20% Efficacy','40% Efficacy'),
              names.arg = grif.name[c(4,5,1,2,3)], yaxt = 'n', xlab= 'Number of averted deaths in thousand',
              args.legend = list(x = "topright",bty = 'n',cex = 1.7), 
              main = paste0(uptake.name[uptake.id],' Uptake'))
      axis(2, at=apply(bp,2,mean), labels=grif.name[c(4,5,1,2,3)], tick=FALSE, las=2, line=-0.8, cex.axis = 1.4, cex.lab = 2)
      text(y = bp, x = avert_matdeath/1000, label = signif(avert_matdeath/1000,3), pos = 4, cex = 1.5, col = "red")
...
## Figures1.4: Cumulative averted financial costs due to BOHEMIA ivermectin MDA for each country
Given a Griffin scenario, the treatment costs averted due to BOHEMIA iMDA. These figures show the marginal effect of BOHEMIA iMDA.\
```{r, results = "asis", echo=F, fig.width=10, fig.height=6, fig.fullwidth=TRUE}
for (country.data.id in 1:9){
  png(paste0(figure_folder, "1.4_averted_cost_",country.name[country.data.id],".png"), 600, 1000)
  par(mfrow=c(3,1))
  ...
  for (grif.rt.id in 1:5){
    ...
    for (eff.id in 1:2){
      for (sev.case.id in 1:2){
        avert.temp<-temp[[uptake.id]][[eff.id]][[grif.rt.id]][[sev.case.id]][[country.data.id]][[2]]
        bardata.temp<-c(bardata.temp,avert.temp[length(avert.temp)])
      }
    }
    bardata[[grif.rt.id]]<-bardata.temp
  ...
  bp<-barplot(bar_mat/1000000,horiz = TRUE,beside = T,xlim=xlim, cex.axis = 1.5, cex.lab = 1.5, cex.main=1.5,
              legend.text = c('20% Efficacy, 1% severe cases','20% Efficacy, 3% severe cases',
                              '40% Efficacy, 1% severe cases','40% Efficacy, 3% severe cases'),
              names.arg = grif.name[c(4,5,1,2,3)], yaxt = 'n', xlab= 'Dollars (million)',
              args.legend = list(x = "topright",bty = 'n', cex=1.7), 
