...
- Instead, we can run the following script in sql: `mysql -updata -pdata openhds`. Note, this script was generated by the code in `bohemia/R/openhds_create_location_hierachies.R`
```
  ...
  - Take the following steps in the web interface of the OpenHDS server at https://bohemia.team/openhds
    - Log in as admin/test to open OpenHDS Server
        - Click on menu item "Configuration" - User management
          ...
          - Log out
          - Log in as data/data
    - Click on menu item "Configuration" - Field Workers
